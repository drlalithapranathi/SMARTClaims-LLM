#!/usr/bin/env python3
"""
Experiment: no unknowns — pure radiology CPT SFT.
Ablation vs exp_unk10_ep2 (10% unknowns).
Base model: qwen3-32b-mimic-cpt-merged (same as unk10)

Run:
    accelerate launch --num_processes 3 --mixed_precision bf16 train_sft_nounk.py
"""
...
SFT_TRAIN_DATA  = "nounk_sft_train"
SFT_VAL_DATA    = "nounk_sft_val"
OUTPUT_DIR      = "nounk_sft_out"
SAVE_ADAPTER    = "nounk_sft_adapter"
SAVE_FULL_MODEL = "nounk_sft_merged"
...
LORA_RANK      = 16
MAX_SEQ_LENGTH = 4096
WANDB_PROJECT  = "SmartClaims-nounk"
...
if accelerator.is_main_process:
    print(f"\n{'='*60}")
    print(f"  SFT — 0% unknowns (pure radiology), 2 epochs, DDP {world_size} GPUs")
    print(f"  Base model : {BASE_MODEL}")
    print(f"  Train data : {SFT_TRAIN_DATA}")
    print(f"  Adapter    : {SAVE_ADAPTER}")
    print(f"  Merged     : {SAVE_FULL_MODEL}")
    print(f"  Eff. batch : {BATCH_SIZE} x {GRAD_ACCUM} x {world_size} = {BATCH_SIZE * GRAD_ACCUM * world_size}")
    print(f"{'='*60}\n")
...
if accelerator.is_main_process:
    ...
    wandb.init(
        ...
        name=f"sft-nounk-r{LORA_RANK}-DDP{world_size}gpu",
        config={
...
if accelerator.is_main_process:
    ...
    print("\n" + "="*60)
    print("  SFT COMPLETE (0% unknowns, 2 epochs)")
    print(f"  Adapter : exp_nounk/{SAVE_ADAPTER}/")
    print(f"  Merged  : exp_nounk/{SAVE_FULL_MODEL}/")
    print("="*60)
