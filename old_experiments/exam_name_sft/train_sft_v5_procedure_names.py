#!/usr/bin/env python3
"""
SFT of Qwen3-32B on MIMIC-IV radiology procedure identification.
Starts from ep2 CPT adapter (frozen), adds fresh SFT LoRA on top.
Saves SFT adapter + full merged model.

Run:
    accelerate launch --num_processes 2 --mixed_precision bf16 train_sft_v5_procedure_names.py
"""
...
TRAIN_JSONL     = "mimic_radiology_sft_train_cleaned.jsonl"
VAL_JSONL       = "mimic_radiology_sft_val_cleaned.jsonl"
...
if accelerator.is_main_process:
    ...
    print(f"\nGPU {local_rank}: {alloc:.1f} GB / {total:.1f} GB ({total-alloc:.1f} GB free)\n")
...
train_dataset = train_dataset.map(keep_messages, remove_columns=["hadm_id"])
val_dataset   = val_dataset.map(keep_messages,   remove_columns=["hadm_id"])
