#!/usr/bin/env python3
"""
SFT v7 of Qwen3-32B on MIMIC-IV radiology → CPT code prediction.
v7 changes vs v6:
  - Labels are CPT codes (not procedure names)
  - Training data includes 15% discharge summaries with 'unknown' label
  - Loads pre-built parquet dataset (from build_sft_grpo_datasets.py)
...
Run:
    accelerate launch --num_processes 4 --mixed_precision bf16 train_sft_v8.py
"""
...
SFT_TRAIN_DATA  = "sft_v7_train_dataset"
SFT_VAL_DATA    = "sft_v7_val_dataset"
OUTPUT_DIR      = "outputs_sft_v8"
SAVE_ADAPTER    = "qwen3-32b-sft-v8"
SAVE_FULL_MODEL = "qwen3-32b-sft-v8-merged"
...
LORA_RANK      = 16
MAX_SEQ_LENGTH = 4096
WANDB_PROJECT  = "SmartClaims-SFT-v8"
...
if accelerator.is_main_process:
    print(f"\n{'='*60}")
    print(f"  SFT v8 — DDP on {world_size} GPUs")
...
if accelerator.is_main_process:
    ...
    wandb.init(
        ...
        name=f"qwen3-32b-sft-v8-r{LORA_RANK}-{EPOCHS}ep-DDP{world_size}gpu",
        config={
            "model":           BASE_MODEL,
            "sft_strategy":    "cpt_merged_base + fresh_sft_lora",
            "data_version":    "v7 — CPT codes as labels, 15% discharge/unknown",
            "lora_r":          LORA_RANK,
            "lora_dropout":    0.05,
        ...
        tags=["stage2", "sft-v8", "mimic-iv", "qwen3-32b", "cpt-codes", "ddp"],
    )
